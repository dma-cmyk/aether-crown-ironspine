...
# Written as RGBA (PNG32): grey+alpha PNGs come out black in the Compatibility renderer used on the Web.
set -e
...
magick "$T/smoke_rgb.png" "$T/smoke_a.png" -alpha off -compose CopyOpacity -composite "PNG32:$OUT/fx_smoke.png"
# glow: bright core + wide falloff
magick -size ${S}x${S} radial-gradient:white-black -evaluate pow 3.2 "$T/glow_a.png"
magick -size ${S}x${S} xc:white "$T/glow_a.png" -alpha off -compose CopyOpacity -composite "PNG32:$OUT/fx_glow.png"
...
magick -size ${S}x${S} xc:white "$T/flash_a.png" -alpha off -compose CopyOpacity -composite "PNG32:$OUT/fx_flash.png"
# fire: noisy flame blob
magick "$T/n2.png" "$T/round.png" -compose multiply -composite -level 10%,55% "$T/fire_a.png"
magick -size ${S}x${S} xc:white "$T/fire_a.png" -alpha off -compose CopyOpacity -composite "PNG32:$OUT/fx_fire.png"
...
magick -size 256x256 xc:"#1a1612" "$T/sc_a.png" -alpha off -compose CopyOpacity -composite "PNG32:$OUT/fx_scorch.png"
rm -rf "$T"
ls -la "$OUT"
