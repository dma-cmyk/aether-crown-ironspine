...
const BUILD := """[color=#d0a85c]生産[/color]
  兵舎・工廠・飛行場（飛行艦・機動兵・空中戦艦）・祠を選択 → Q W E R … ユニット生産（Shift+クリックで5体）
...
[color=#d0a85c]建設[/color]
  ...
  建設できるのは本拠地・城門・自軍の都市の周辺だけ。建てる場所を選んでいる間は地面にマス目が出る：
  [color=#6fe08a]緑[/color] 建てられる　[color=#f2d24a]黄[/color] 木を切り倒して建てられる（木も黄色くなる）　[color=#ff6a5a]赤[/color] 岩・建物・急な斜面で建てられない
  木の上に置くと、建設が始まるときにその木が倒れて地面が通れるようになる。置けないときは理由が建物の上に出る。
...
const TOUCH_BUILD := """[color=#d0a85c]生産[/color]
  兵舎・工廠・飛行場（飛行艦・機動兵・空中戦艦）・祠を選択 → 右下のボタンでユニットを生産（数字は順番待ちの数）
  生産中のユニットのアイコンをタップすると取り消せる。建物を選んで地面をタップした所が集結地点になる。

[color=#d0a85c]建設[/color]
  本拠地を選択 →「建設」→ 建物のボタンを押したまま地面へ引き出して、指を離すとそこに建設（祠と天罰の塔は「祠・塔」の中）
  仮置きした建物は指で引いて動かせる（離すと建設）。ボタンをタップ → 地面をタップで仮置き → もう一度タップで建設、もできる
  建設できるのは本拠地・城門・自軍の都市の周辺だけ。建てる場所を選んでいる間は地面にマス目が出る：
  [color=#6fe08a]緑[/color] 建てられる　[color=#f2d24a]黄[/color] 木を切り倒して建てられる（木も黄色くなる）　[color=#ff6a5a]赤[/color] 岩・建物・急な斜面で建てられない
  木の上に置くと、建設が始まるときにその木が倒れて地面が通れるようになる。置けないときは理由が建物の上に出る。
