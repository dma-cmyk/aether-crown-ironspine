...
[color=#d0a85c]生産と建設[/color]
  兵舎・工廠・飛行場・祠を選択 → Q W E R … ユニット生産（Shift+クリックで5体）
  本拠地を選択 → B または「建設」… 建設メニュー（Q〜Y で建物を選び、地面をクリック。U または「祠」で祠の一覧）
  建設できるのは本拠地・城門・自軍の都市の周辺だけ。
  神獣・悪魔・天使は種類ごとの祠（建設メニューの「祠」）で呼ぶ。機械ではないので修理はできないが、戦闘から離れると自然に回復する。
...
[color=#d0a85c]生産と建設[/color]
  兵舎・工廠・飛行場・祠を選択 → 右下のボタンでユニットを生産（数字は順番待ちの数）
  本拠地を選択 →「建設」→ 建物を選ぶ（祠は「祠」の中） → 地面をタップで仮置き、仮置きした建物をもう一度タップで建設
  建設できるのは本拠地・城門・自軍の都市の周辺だけ。
  神獣・悪魔・天使は種類ごとの祠（建設メニューの「祠」）で呼ぶ。機械ではないので修理はできないが、戦闘から離れると自然に回復する。
