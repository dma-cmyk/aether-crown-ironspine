...
- 建物: `citadel` `barracks` `foundry` `skyport` `refinery` `habitat` `bastion` `sanctum_cerberus` `sanctum_cyclops` `sanctum_griffin` `sanctum_dragon`（神獣ごとの祠） `gate`。`sanctum` は全種を呼べる旧版の祠で、建設メニューには出ない
