...
内蔵の「アイアンスパインの門」（`game/scenarios/hold_the_gate.json`）も同じ形式なので、実例として参照してください。
...
文章の中の `{var:名前}` は変数の値、`{countdown:秒}` はその時刻までの残り時間（分:秒）、`{sites:チーム}` はそのチームの都市の数、
